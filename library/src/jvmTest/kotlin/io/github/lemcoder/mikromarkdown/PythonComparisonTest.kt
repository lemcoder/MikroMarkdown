...
class PythonComparisonTest {
    ...
    @Test fun testBlogHtml() = compare("test_blog.html")

    @Test fun testWikipediaHtml() = compare("test_wikipedia.html")

    private fun compare(filename: String) {
        val url =
            javaClass.classLoader.getResource("test_files/$filename")
    ...
    private fun tokenize(text: String): Set<String> =
        normalize(text).lowercase().split(Regex("[\\s\\p{Punct}]+")).filter { it.length > 3 }.toSet()
    ...
    companion object {
        ...
        private fun resolveMarkitdownCmd(): List<String> {
            ...
            return candidates.firstOrNull { cmd ->
                try {
                    val process = ProcessBuilder(cmd + "--help").redirectErrorStream(true).start()
        ...
        fun runMarkitdown(path: String): String {
            val process = ProcessBuilder(markitdownCmd + path).start()
            val stdout = process.inputStream.bufferedReader().readText()
            val finished = process.waitFor(30, TimeUnit.SECONDS)
            assumeTrue(finished, "markitdown timed out on $path")
            assumeTrue(
                process.exitValue() == 0,
                "markitdown failed on $path: ${process.errorStream.bufferedReader().readText()}",
            )
            return stdout
        }
    }
